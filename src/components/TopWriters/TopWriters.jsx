...
const TopWriters = ({action}) => {
  ...
  return (
    <div className="sideBar-element">
      <h3 className="trending-title">Top writers <BsFeather style={{fontSize: '1rem'}} /></h3>
      {usersToDisplay.map((user) => (
        <Link key={user.id} to={`/${user.name}` }>
          <div className='trending-writers'>
            <img className='profile-info-pic' src={user.profilePic} alt="trending-profile-picture" />
